...
if vRPConfig.LootBags then
    ...
    AddEventHandler('vRP:LootBag', function(netid)
        local source = source
        if LootBagEntities[netid] and not LootBagEntities[netid][3] and
            #(GetEntityCoords(LootBagEntities[netid][1]) - GetEntityCoords(GetPlayerPed(source))) < 2.0 then
            ...
            for i, v in pairs(LootBagEntities) do
                if v[5] then
                    ...
                    if #(objectcoords - coords) > 2.0 then
                        TriggerEvent('vRPInv:ChestClientCloseMenuSource', v[5])
                        TriggerClientEvent('vRPInv:ToggleInventory', v[5])
                        Wait(3000)
                        v[3] = false;
                        v[5] = nil;
                    end
    ...
    function OpenInv(source, netid, items)
        ...
        local cb_take_amount = function(idname, amount)
            local citem = LootBagEntities[netid].Items[idname]
            if not citem or citem.amount <= 0 then
                LootBagEntities[netid].Items[idname] = nil
                return;
            end
            if amount == "all" then amount = citem.amount end
            amount = parseInt(amount)
            local new_weight = vRP.getInventoryWeight(user_id) + vRP.getItemWeight(idname) * amount
            if new_weight <= vRP.getInventoryMaxWeight(user_id) then
                vRP.giveInventoryItem(user_id, idname, amount, true)
                ...
                TriggerEvent('vRPInv:UpdateBothInventory', source, 'LootBag|' .. netid, OtherItems, 50)
            end
        end
        ...
        TriggerEvent('vRPInv:OpenLootBag', source, items, netid, cb_take, cb_close, cb_take_amount)
    end
end
