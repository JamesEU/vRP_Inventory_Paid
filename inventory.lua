...
function vRP.getItemWeightProxy(idname)
  local args = vRP.parseItem(idname)
  local item = vRP.items[args[1]]
  if item ~= nil then return vRP.computeItemWeight(item,args) end
  return 0
...
function vRP.getItemWeightreal(idname)
  local args = vRP.parseItem({idname})
  local item = vRP.items[args[1]]
  if item ~= nil then return vRP.computeItemWeight({item,args}) end
  return 0
...
-- compute weight of a list of items (in inventory/chest format)
function vRP.computeItemsWeightreal(items)
  local weight = 0
    ...
    if data and data.inventory then
      if not chests[name] then
        ...
        vRP.getSData("chest:"..name, function(cdata)
          ...
          local function takeItem(idname, amount)
            local citem = chest.items[idname]
            if citem and amount >= 0 and amount <= citem.amount then
              local new_weight = vRP.getInventoryWeight(user_id) + vRP.getItemWeight(idname) * amount
              if new_weight <= vRP.getInventoryMaxWeight(user_id) then
                vRP.giveInventoryItem(user_id, idname, amount, true)
                citem.amount = citem.amount - amount
                ...
                if cb_out then cb_out(idname, amount) end
                local Items = {}
                for i,v in pairs(chest.items) do 
                  Items[i] = {
                ...
                TriggerEvent('vRPInv:UpdateBothInventory', source, name, Items, max_weight)
              else
                vRPclient.notify(source, {lang.inventory.full()})
              end
            else
          ...
          local function putItem(idname, amount)
            local new_weight = vRP.computeItemsWeight(chest.items) + vRP.getItemWeightProxy(idname) * amount
            if new_weight <= max_weight then
              if amount >= 0 and vRP.tryGetInventoryItem(user_id, idname, amount, true) then
                local citem = chest.items[idname]
                if citem then
          ...
          local function closeChest(source, name)
            if chests[name] then
              TriggerClientEvent("vRPInv:closeChest", source, name)
              vRP.setSData("chest:"..name, json.encode(chest.items))
              chests[name] = nil
              TriggerEvent("vRPInv:ChestClosedClientNotify", source, name)
          ...
          TriggerClientEvent("vRPInv:openChest", source, name, Items, max_weight)
          TriggerEvent("vRPInv:ChestTracker", source, name, cb_close, cb_in, cb_out, putItem, takeItem, closeChest, getChestItems)
          SetTimeout(300000, function()
            if chests[name] then
              TriggerClientEvent("vRPInv:closeChest", source, name)
              vRP.setSData("chest:"..name, json.encode(chest.items))
